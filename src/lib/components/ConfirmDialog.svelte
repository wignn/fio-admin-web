...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 backdrop-blur-sm">
		<div class="w-full max-w-md rounded-2xl border border-border bg-surface p-5 shadow-xl animate-fade-in">
			...
			<div class="mt-6 flex justify-end gap-2">
				...
				<button
					type="button"
					class="rounded-lg px-4 py-2 text-sm font-semibold text-white transition disabled:opacity-60 {confirmTone === 'red' ? 'bg-red hover:bg-red/90' : 'bg-accent hover:bg-accent-glow'}"
					onclick={confirm}
					disabled={busy}
				>
